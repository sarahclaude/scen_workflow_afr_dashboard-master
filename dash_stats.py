...
def calc_clusters(
    ...
    # Identify the simulations that are shared between variables.
    ...
    for i in range(cntx.varidxs.count):
        ...
        df_i = load()
        # df_i = df_i.loc[[sim for sim in list(df_i.index) if sim in sim_l]]
        if df_rel is None:
            df_rel = df_i[[column_rep]]
            df_rel.columns = [cntx.varidx.code]
        df_rel[cntx.varidx.code] = df_i[column_rep]
    df_rel = df_rel.dropna()
    df_abs = df_abs.dropna()

    # Perform clustering.
    ac = AgglomerativeClustering(n_clusters=int(n_cluster), affinity="euclidean", linkage="ward").fit(df_rel)
    groups = ac.fit_predict(df_rel)

    # Format table.
    df_format = pd.DataFrame(df_rel.index)
